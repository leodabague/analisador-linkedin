...
import { findCanonicalFromAliases } from './utils/companyAliases';
import { normalizeCompanyName, getCompanyTokens, calculateSimilarity, clusterCompanies, extractCompanyFromTitle } from './utils/companyUtils';
...
const LinkedInInsightsAnalyzer = () => {
  ...
  const companyClustering = useMemo(() => {
    return clusterCompanies(companyExtraction);
  }, [companyExtraction]);
  ...
  const peopleAnalysis = useMemo(() => {
    ...
    return validPeople.filter(person => {
      ...
      const company = extractCompanyFromTitle(person.title).toLowerCase();
          ...
          {(() => {
            ...
            return (
              <>
                <div className="space-y-2">
                  {pageItems.map((person, i) => {
                    const company = extractCompanyFromTitle(person.title);
